...
static const uint16 kDefaultKbdControls[kKeys_Total] = {
  ...
  // CheatLife, CheatKeys, ClearKeyLog, StopReplay, Fullscreen, Reset, Pause, PauseDimmed, Turbo, ZoomIn, ZoomOut, MasterVolumeDown, MasterVolumeUp,
  _(SDLK_w), _(SDLK_o), _(SDLK_k), _(SDLK_l), A(SDLK_RETURN), _(SDLK_e), S(SDLK_p), _(SDLK_p), _(SDLK_t), N, N, C(SDLK_LEFTBRACKET), C(SDLK_RIGHTBRACKET),
};
#undef _
#undef A
#undef C
...
static const KeyNameId kKeyNameId[] = {
  ...
  S(Pause), S(PauseDimmed), S(Turbo), S(ZoomIn), S(ZoomOut), S(MasterVolumeDown), S(MasterVolumeUp),
};
#undef S
#undef M
